...
from app.services.broker_connection_service import (
    ...
    get_broker_connection_by_id,
    update_broker_connection,
    delete_broker_connection
)

router = APIRouter(
    prefix="/broker-connections",
...
def get_broker_connections_endpoint(
    db: Session = Depends(get_db)
):
...
def get_broker_connection_by_id_endpoint(
    ...
    connection = get_broker_connection_by_id(
        db,
        connection_id
    )
...
def delete_broker_connection_endpoint(
    ...
    deleted_connection = delete_broker_connection(
        db,
        connection_id
    )
