...
from app.services.telegram_notification_service import (
    ...
    get_telegram_notification_by_id,
...
@router.get(
    "/",
    response_model=list[TelegramNotificationResponse]
)
def get_telegram_notifications_endpoint(
    db: Session = Depends(get_db)
):
...
def get_telegram_notification_by_id_endpoint(
    ...
    notification = get_telegram_notification_by_id(
        db,
        notification_id
    )
    ...
    if not notification:
        raise HTTPException(
            status_code=404,
            detail="Telegram notification not found"
        )
...
def delete_telegram_notification_endpoint(
    ...
    deleted_notification = delete_telegram_notification(
        db,
        notification_id
    )
